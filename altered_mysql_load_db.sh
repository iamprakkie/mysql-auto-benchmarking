  ...
  for TABLE in $TABLES ; do
    ...
    if [ "x$ONLY_ITEM" = "x" ] ; then
      if [ "$TABLE" = "HISTORY" -o "$TABLE" = "STOCK" -o "$TABLE" = "ORDERS" -o "$TABLE" = "ORDER_LINE" ]; then
        ...
        command_exec "$MYSQL $DB_NAME -e \"\$$TABLE $NDB_DISK_DATA ENGINE=$DB_ENGINE $LATIN1 $PARTITION_SYNTAX\""
      else
        if [ "x$TABLE" != "xITEM" ] ; then
          echo "Creating table $TABLE in $DB_ENGINE"
          command_exec "$MYSQL $DB_NAME -e \"\$$TABLE ENGINE=$DB_ENGINE $LATIN1 $PARTITION_SYNTAX\""
        fi
      fi
    fi
    if [ "x$TABLE" = "xITEM" ] ; then
      if [ "x$USE_MYISAM_FOR_ITEM" = "x" ] ; then
        ...
        command_exec "$MYSQL $DB_NAME -e \"\$$TABLE ENGINE=MYISAM $LATIN1 $PARTITION_SYNTAX\""
      fi
    fi
  done
fi
...
if [ "$LOCAL" != "" ]; then
  MYSQL_ARGS="$MYSQL_ARGS --local-infile=1"
fi
